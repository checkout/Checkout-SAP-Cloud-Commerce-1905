...
<form id="payment-form" method="POST" action="https://merchant.com/charge-card">
    ...
    <div class="date-and-code">
        <div>
            <label for="expiry-date"><spring:theme code="checkoutcom.multi.expirydate.label"
    ...
    <div class="form-billing-address">
        ...
        <c:if test="${not empty billingAddress}">
            <c:if test="${not empty billingAddress.titleCode}">${fn:escapeXml(billingAddress.titleCode)}&nbsp;</c:if><span
                id="billingFirstName">${fn:escapeXml(billingAddress.firstName)}</span>&nbsp;<span id="billingLastName">${fn:escapeXml(billingAddress.lastName)}</span>
            </br><span id="billingLine1">${fn:escapeXml(billingAddress.line1)}</span>,&nbsp;<c:if
                test="${not empty billingAddress.line2}"><br/><span
                id="billingLine2">${fn:escapeXml(billingAddress.line2)}</span>,&nbsp;</c:if>
            <span id="billingCity">${fn:escapeXml(billingAddress.town)}</span>,&nbsp;<c:if
                test="${not empty billingAddress.region.name}"><span
                id="billingState">${fn:escapeXml(billingAddress.region.name)}</span>&nbsp;</c:if><span
                id="billingPostCode">${fn:escapeXml(billingAddress.postalCode)}</span>
            ...
            <c:if test="${not empty billingAddress.phone }"><br/><span
                    id="billingPhone">${fn:escapeXml(billingAddress.phone)}</span></c:if>
